...
class PersonDetector(object):
    def __init__(self,model_path,threshold=0.3):
        ...
        # dataset
        detect_model_name = model_path

        PATH_TO_CKPT = detect_model_name + '/frozen_inference_graph.pb'

        # setup tensorflow graph
        self.detection_graph = tf.Graph()
        #Seesion later used for NMS
    ...
    def get_localization(self, image, visual=False):
        ...
        with self.detection_graph.as_default():
              ...
              # The ID for car is 3
              idx_vec = [i for i, v in enumerate(cls) if ((v==1) and (scores[i]>self.threshold))]
              temp_boxes=[]
              if len(idx_vec) !=0:
                  person_boxes=[]
                  person_scores=[]
                  for idx in idx_vec:
                      ...
                      if (box_h>20) and (box_w>20):
                          ...
                          person_scores.append(scores[idx])


                  if len(person_boxes)!=0:
                      # print("Detection found\n")
                      temp_boxes,scores=non_max_suppression_with_tf(self.sess,person_boxes,person_scores,max_output_size=1,iou_threshold=0.3)
                      for box,score in zip(temp_boxes,scores):
                          box_h = box[2] - box[0]
                          box_w = box[3] - box[1]
                          ratio = box_h/(box_w + 0.01)
                          # print(box, ', confidence: ', score, 'ratio:', ratio)
                      '''
                      else:
