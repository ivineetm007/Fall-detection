...
# '''
# This function is used to copy the IUV images to the new location
# '''
# def move_and_rename_dir(in_dir,out_dir):
#     os.makedirs(out_dir, exist_ok=True)
#     print("Directory ",in_dir)
#     image_path_list=glob.glob(in_dir+'/*_IUV*')
#     image_list=[path.split('/')[-1] for path in image_path_list]
#     for image_name in image_list:
#         part=image_name.split('_IUV')
#         copy_name=part[0]+part[1]
#         inp_path=os.path.join(in_dir,image_name)
#         out_path=os.path.join(out_dir,copy_name)
#         # print(out_path)
#         try:
...
#
#
# def threshold_dir(in_dir,out_dir,kernel_size=5):
#     os.makedirs(out_dir, exist_ok=True)
...
#                 img_opening = cv2.morphologyEx(binary, cv2.MORPH_OPEN, kernel)
#                 cv2.imwrite(out_path,img_opening)
# def image_rescaling(img,pl,ph):
#     pl, ph = np.percentile(img, (pl, ph))
#     return exposure.rescale_intensity(img, in_range=(pl, ph))
#
#
# def contrast_strecthing_dir(in_dir,out_dir):
#     os.makedirs(out_dir, exist_ok=True)
#     pl,ph=50,99
#     for name in os.listdir(in_dir):
#         # Read image
...
#                 img = np.asanyarray(img)
#                 # result = Image.fromarray((binary * 255).astype(np.uint8))
#                 rescaled=image_rescaling(img,pl,ph)
#                 # # result.save(out_path)
#                 cv2.imwrite(out_path,rescaled)
#
# '''
# This function recursively walk through all directories and aplly function function_dir
# in_dir=root_drive,'Thermal','Fall'
# out_dir=root_drive,'Thermal','Fall_thres'
# '''
#
