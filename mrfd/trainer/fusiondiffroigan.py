    ...
    def train(self,  thermal_frame, thermal_mask,thermal_diff_masks,flow_frame, flow_mask,epochs= 500,epochs_trained=0, save_interval = 10):
        ...
        for epoch in range(epochs_trained+1,epochs):
            ...
            if epoch % save_interval == 0 or epoch == epochs-1:
                ...
                #AE RE plots
                total_RE=[self.train_par.thermal_lambda_S*r_loss_list_S_RE[i]+self.train_par.thermal_lambda_T*r_loss_list_T_RE[i]\
                          + self.train_par.flow_lambda*r_loss_list_F_RE[i] for i in range(len(r_loss_list_S_RE))]
                save_multiple_graph(x_list=[r_loss_list_S_RE,r_loss_list_F_RE,r_loss_list_T_RE,total_RE],\
                                    labels=['S_RE','F_RE','T_RE','Total_RE'],x_label='Batches',y_label='Losses',title='Loss Plot',path=loss_root+'/R_log_losses.png',log_plot=True)


    def plot_images_3D(self, save2file=False,  samples=16, step=0, test_window = None,test_masks=None,diff_masks=None,d_type=None):
        '''
        ...
        if d_type=='thermal':
            ...
            rec_images = self.T_R.predict([test_window,test_masks,diff_masks])
...
#         plt.subplots_adjust(wspace=0,hspace=0)
    ...
    def test(self, test_videos, score_type = 'R', epochs = None,plot=False,tolerance_limit=8):
        ...
        choose score type from ['T_R_S','T_R_T', 'F_R','R']
        ...
        for vid_name in  test_videos.keys():
            ...
            if score_type == 'F_R':
                ...
                for index in range(num_sub_videos):
                    ...
                    RE_dict, recons_seq = self.get_T_S_RE_all_agg(thermal_data=test_data_masked_windowed,thermal_masks=test_mask_windowed,diff_masks=test_diff_mask_windowed) #Return dict with value for each score style
                    in_mean_RE.append(RE_dict['in_mean'])
                    in_std_RE.append(RE_dict['in_std'])
                ...
                for index in range(num_sub_videos):
                    ...
                    #Get metric value
                    RE_dict, recons_seq = self.get_T_T_RE_all_agg(thermal_data=test_data_masked_windowed,thermal_masks=test_mask_windowed,diff_masks=test_diff_mask_windowed) #Return dict with value for each score style
                    in_mean_RE.append(RE_dict['in_mean'])
                    in_std_RE.append(RE_dict['in_std'])
                ...
                #Thermal spatial RE
                RE_dict, recons_seq = self.get_T_S_RE_all_agg(thermal_data=np.concatenate(thermal_data_windowed_list),thermal_masks=np.concatenate(thermal_mask_windowed_list),diff_masks=np.concatenate(diff_mask_windowed_list)) #Return dict with value for each score style
                in_mean_T_S_RE = RE_dict['in_mean']
                in_std_T_S_RE = RE_dict['in_std']
                #Thermal temporal RE
                RE_dict, recons_seq = self.get_T_T_RE_all_agg(thermal_data=np.concatenate(thermal_data_windowed_list),thermal_masks=np.concatenate(thermal_mask_windowed_list),diff_masks=np.concatenate(diff_mask_windowed_list)) #Return dict with value for each score style
                in_mean_T_T_RE = RE_dict['in_mean']
                in_std_T_T_RE = RE_dict['in_std']

                #Flow RE
                RE_dict, recons_seq = self.get_F_RE_all_agg(flow_data=np.concatenate(flow_data_masked_windowed_list),flow_masks=np.concatenate(flow_mask_windowed_list)) #Return dict with value for each score style
                in_mean_F_RE = RE_dict['in_mean']
                in_std_F_RE = RE_dict['in_std']
                #Total AE RE
                final_in_mean =  self.train_par.thermal_lambda_S*in_mean_T_S_RE+self.train_par.thermal_lambda_T*in_mean_T_T_RE+self.train_par.flow_lambda*in_mean_F_RE
                final_in_std =  self.train_par.thermal_lambda_S*in_std_T_S_RE+self.train_par.thermal_lambda_T*in_std_T_T_RE+self.train_par.flow_lambda*in_std_F_RE
