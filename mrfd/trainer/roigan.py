...
class ROI_3DCAE_GAN3D(object):
    ...
    def initialize_model(self,Reconstructor , Discriminator ):
        ...
        self.stacked_R_D.compile(loss={'D': 'binary_crossentropy', 'decoded': ROI_mean_squared_error_loss(self.R.input[1])},\
    ...
    def get_MSE_all_agg(self, test_data, test_masks,type=['frame','window']):
        """
        ...
        if self.train_par.d_type=='ROIopticalFLow':
            win_length=win_length-1
        recons_seq = model.predict([test_data,test_masks]) #(samples-win_length+1, win_length, wd,ht,1)
    ...
    def train(self, X_train_frame, X_train_mask,epochs= 500,epochs_trained=0, save_interval = 10):
        '''
        ...
        for epoch in range(epochs_trained+1,epochs):
            ...
            for step in range(num_batches):
                ...
                #R Input
                recons_images = self.R.predict([legit_images,masks])
                ...
                r_loss = self.stacked_R_D.train_on_batch([legit_images,masks], {'decoded':legit_images,'D':y_mislabled})
    ...
    def plot_images_3D(self, save2file=False,  samples=16, step=0, test_window = None,test_masks=None,d_type=None):
        ...
        if d_type=='thermal':
            ...
            rec_images = self.R.predict([test_window,test_masks])
    ...
    def test(self, test_videos, score_type = 'R', epochs = None,plot=False,tolerance_limit=8):
        ...
        for vid_name in  test_videos.keys():
            ...
            if self.train_par.d_type=='ROIframe':
                ...
                test_mask_windowed_list = [create_windowed_arr(test_data, stride, win_length) for test_data in test_mask_list]
                ...
                # creating windows of mask data
                test_mask_list = [vid.reshape(len(vid), img_width, img_height, 1) for vid in vid_mask_list]
                test_mask_list=[np.concatenate((vid,vid,vid),axis=-1) for vid in test_mask_list]
                test_thermal_mask_windowed_list = [create_windowed_arr(test_data, stride, win_length).astype('int8') for test_data in test_mask_list]


                # creating diff mask data/flow masks Union of consecutive thermal ROI
                test_mask_windowed_list=[create_diff_mask(mask_windows) for mask_windows in test_thermal_mask_windowed_list]
                # Masking the flow data
                test_data_windowed_list=[(test_data_windowed_list[i]*test_mask_windowed_list[i])+test_mask_windowed_list[i]-1 for i in range(len(test_data_windowed_list))]
            else:
            ...
            # print("Number of sub mask videos,",len(test_mask_windowed_list))
            ...
            if score_type == 'R':
                ...
                for index in range(num_sub_videos):
                    ...
                    RE_dict, recons_seq = self.get_MSE_all_agg(test_data_windowed,test_mask_windowed) #Return dict with value for each score style
            ...
            if plot == True and score_type == 'R' and self.train_par.d_type=='ROIframe':
